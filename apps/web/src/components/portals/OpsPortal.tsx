...
export function OpsPortal({ user, onSignOut }: { user: any; onSignOut: () => void }) {
  const [view, setView] = useState<'trips' | 'wagons' | 'requests' | 'analytics'>('trips');

  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      <header className="bg-slate-900 text-white p-4 flex justify-between items-center">
        <div>
          <h1 className="text-lg font-black tracking-wider">BUENO FREIGHT OS — OPERATIONS DISPATCH HQ</h1>
          <p className="text-xs text-slate-400">Supervising Head of Operations: {user.fullName}</p>
        </div>
